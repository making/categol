...
(defmacro define-crud-dispatcher (target)
  "define dispatcher for the routes of crud pages

   Ex. define foo crud dispatcher

   [route]                [call]
   /view/xxx/yyy/zzz   -> (foo-veiw (xxx yyy zzz))
   /edit/xxx/yyy/zzz   -> (foo-edit (xxx yyy zzz))   ; needs authentication
   /create/xxx/yyy/zzz -> (foo-create (xxx yyy zzz)) ; needs authentication
   /delete/xxx/yyy/zzz -> (foo-delete (xxx yyy zzz)) ; needs authentication
   otherwise           -> (foo-veiw (xxx yyy zzz))
  "
  (let ((root-string (gensym))
        (root (gensym))
        (set-from-sexp `(when-hunchentoot ()
                          (setf (hunchentoot:session-value +session-from-key+) 
                                (append (list ',target) path-info)))))
...
(defun blog-dispatch-controller (&optional (path-info (path-info)))
  "the root dispatcher of this blog sysytem.
   only this dispatcher is registered to hunchentoot:*dispatch-table*.
  "
